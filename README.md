...
> [!TIP]
> Для максимально красивой карточки репозитория можешь заменить GIF на записи своего интерфейса, сохранив те же имена файлов.
...
flowchart TD
    ...
    B --> C[Нажата кнопка "Зашифровать"]
    C --> D[Ввод обычного текста]
    D --> E[Токен SHIFR1.*]

    B --> F[Нажата кнопка "Расшифровать"]
    F --> G[Ввод токена SHIFR1.*]
    G --> H{Проверка HMAC}
    H -- OK --> I[Исходный текст]
    H -- FAIL --> J[Ошибка: невалидный шифр]
```
